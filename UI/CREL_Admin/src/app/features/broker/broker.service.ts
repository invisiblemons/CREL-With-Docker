...
export class BrokerService {
  ...
  getBrokers(): Observable<any> {
    return this.httpClient.get<Broker[]>(
      `${this.brokerURL}?OrderBy=2&PageSize=100`
    );
  }

  getActiveBrokers(): Observable<any> {
    return this.httpClient.get<Broker[]>(
      `${this.brokerURL}?MinStatus=1&OrderBy=2&PageSize=100`
    );
  }

  getBrokersOfTeam(teamId): Observable<any> {
    return this.httpClient.get<Broker[]>(
      `${this.brokerURL}?Mintatus=1&OrderBy=2&PageSize=100&TeamId=${teamId}`
    );
  }
  ...
  getBrokerByUserName(userName): Observable<Broker[]> {
    return this.httpClient.get<Broker[]>(
      `${this.brokerURL}?UserName=${userName}&OrderBy=2`
    );
  }
